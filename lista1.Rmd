...
r = 1 # registro relacionado a cada uma das amostras,
      # para média amostral 
	# xbar[r] = mean(x), constante de Gini ghatC[r] = 1/(n^2*xbar[r]), 
	# e o somatório de 1 a n somaAmost[i]=(2*i-n-1)*xi[i],
  # da fórmula de Gini 
	# como função das estatísticas de ordem.
...
while(j<=10000){	# cálculo das j (10000) estimativas Gini 
	...
	x=rlnorm(n, meanlog = 0, sdlog = 1) # gera amostra aleatória
	                                    # lognormal
...
table(G<inf) # se TRUE,
             # valor simulado está abaixo do intervalo inferior
a=matrix(table(G<inf))

table(G>sup)	# se TRUE,
              # valor simulado está acima do intervalo superior
