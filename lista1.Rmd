...
j = 1 # contador do número de amostras (escolhi, arbitrariamente, 10000)

r = 1 # registro relacionado a cada uma das amostras, para média amostral 
	# xbar[r] = mean(x), constante de Gini ghatC[r] = 1/(n^2*xbar[r]), 
	# e o somatório de 1 a n somaAmost[i]=(2*i-n-1)*xi[i], da fórmula de Gini 
...
while(j<=10000){	# cálculo das j (10000) estimativas Gini 

	x=rlnorm(n, meanlog = 0, sdlog = 1) # gera amostra aleatória lognormal
	xbar[r]=mean(x)				# calcula x barra
	ghatC[r]=(1/(n^2*xbar[r]))		# calcula a constante

		xi=sort(x)				# ordena a amostra gerada

			somaAmost=0				# laço para cálculo do somatório
			for (i in 1:n)	{			 	
			somaAmost[i]=(2*i-n-1)*xi[i]		
						}			

			somaTot[r]=sum(somaAmost)	# registrando cada um dos somatórios

				G[j]=ghatC[r]*somaTot[r]	# cálculo da estimativa G
...
#### Construindo o IC ((1-alpha)*100%) para G

media=mean(G)
desvio=sd(G)	# desvio padrão das estimativas G
zneg=-1.96		# z para alfa=0.025
zpos=1.96		# z para alfa=0.975

inf=media+zneg*desvio		# limite inferior do intervalo 
sup=media+zpos*desvio		# limite superior do intervalo

table(G<inf)	# se TRUE, valor simulado está abaixo do intervalo inferior
a=matrix(table(G<inf))

table(G>sup)	# se TRUE, valor simulado está acima do intervalo superior
b=matrix(table(G>sup))

#### valor (%) simulado fora do intervalo de 95%

percent=100*((a[2,1]+b[2,1])/10000)	
percent
